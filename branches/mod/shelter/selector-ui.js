(() => {
	...
	updateCurrent().then(() => {
		if (window.Vencord && !currentBranches().includes("vencord") && !currentBranches().includes("equicord") && !currentBranches().includes("pawsome"))
	...
	function BranchEntry(props /*: { name, data, value, onChange } */) {
		...
		const disabled = () => {
			...
			if ((props.name === "vencord" || props.name === "equicord" || props.name === "pawsome") && vencordOtherwiseLoaded()) {
				return "Vencord, Equicord, or PawsomeVencord are currently loaded by some other mechanism.";
			}
